...
使用例:
  ...
  tts-plugin-bridge test                    # デフォルトエンジンでテスト合成
    ...
    synth_parser.add_argument(
        "--engine", "-e", help="使用するTTSエンジン (デフォルト: piperplus)"
    )
    synth_parser.add_argument(
        "--speed", "-s", type=float, default=1.0, help="話速 (0.1-3.0, デフォルト: 1.0)"
    )
    synth_parser.add_argument("--volume", "-v", type=float, help="音量 (0.0-3.0)")
    synth_parser.add_argument(
        "--pitch", "-p", type=float, help="ピッチ補正 (エンジン依存)"
    )
    synth_parser.add_argument(
        "--output", "-o", help="出力ファイルパス (指定しない場合はBase64を表示)"
    )
    synth_parser.add_argument(
        "--play", action="store_true", help="音声を直接再生する (paplay/aplay)"
    )
    synth_parser.add_argument(
        "--server-url", help="TTSサーバーURL (例: http://localhost:5000)"
    )
    synth_parser.add_argument(
        "--style-id", type=int, help="AivisSpeechなど: 話者スタイルID (例: 888753760)"
    )
    synth_parser.add_argument(
        "--chunk", action="store_true", help="テキスト分割を有効にする"
    )
